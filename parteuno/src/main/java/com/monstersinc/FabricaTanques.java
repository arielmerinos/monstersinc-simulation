...
public class FabricaTanques {
  ...
  private int tiempoFabricacion; // Variable para el tiempo de fabricación
                                 // (puede variar)

  public FabricaTanques(int tiempoFabricacion) {
    this.fabricacionLock = new ReentrantLock();
    this.tiempoFabricacion = new Random().nextInt(
        tiempoFabricacion + 1); // simular que cada tanque toma su tiempo
  }
  // ¿? Como cubrir la parte - "Para fabricarlos, se requiere un monstruo para
  // cada uno" Falta la parte de  recoleccion y transporte al almacen
